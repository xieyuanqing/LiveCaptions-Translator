...
- **🧠 可切换ASR来源（MVP）**
  ...
  - **Windows Live Captions**：原始内置链路（基于 UIA）。
  - **Whisper Bridge**：连接本地 WebSocket bridge（推荐用于 WhisperLive 类流式后端）。

  这项改动保留了原有翻译 API 和使用体验，同时允许在日语直播场景中接入更高质量的 ASR 链路。
...
### 可选：使用 Whisper Bridge ASR

如果您想使用 Whisper 系流式识别，请先启动本地 bridge。bridge 需要向 WebSocket 端点持续推送文本增量（应用默认地址：`ws://127.0.0.1:8765/captions`）。

然后在**设置**页中：

1. 将 **ASR Source** 设置为 `Whisper Bridge`。
2. 设置 **Whisper Bridge URL**（默认：`ws://127.0.0.1:8765/captions`）。
3. 按需调整 **Enable Partial** 与 **Reconnect (ms)**。
